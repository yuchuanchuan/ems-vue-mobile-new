...
export default {
      ...
      if (this.orderList && this.orderList.length > 0) {
        ...
        for (var key in this.orderList) {
          if (this.orderList[key]['status'] === 2) {
            let startTime = new Date().getTime()
            let endTime = new Date(new Date(new Date().getTime() - 60 * 60 * 1000).setHours(0, 0, 0, 0)).getTime() +
                86400000
  ...
  methods: {
    ...
    jump () {
      this.$router.push({
        path: `/flow/${localStorage.getItem('openid')}/${localStorage.getItem('areaid')}`
      })
    },
    InitTime (endtime) {
    ...
    cancelOrderTime () {
      // 当天24点的毫秒数
      var endtime = new Date(new Date(new Date().getTime() - 60 * 60 * 1000).setHours(0, 0, 0, 0)).getTime() +
          86400000
        ...
        if (data && data.code === 0) {
          data.data.map((obj, index) => {
            ...
            if (obj.status === 2) {
              ...
              console.log(new Date(obj.createTimestamp).toDateString() + '---------' + new Date().toDateString())
              //
