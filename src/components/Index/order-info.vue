  ...
  export default {
    ...
    methods:{
      ...
      getWxPhoto(){
          ...
          let that = this;
          that.$http({
            url: that.$http.adornUrl('/wechatJs/wechatPhoto'),
            method: 'get',
            params: that.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              alert('appId====' + data.data.appId)
              alert('nonceStr======' + data.data.nonceStr)
              alert('timestamp=====' + data.data.timestamp)
              alert('signature=========' + data.data.signature)
              alert('url===========' + data.data.url)
              wx.config({
                debug: false,
                appId: data.data.appId,
                nonceStr: data.data.nonceStr,
