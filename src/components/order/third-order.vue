<template>
  ...
  <div class="content2">
    ...
    <div class="box">
      <div class="tui" @click="tui1"></div>EMS邮寄信息
    </div>
    <div class="smtitle">
      <div class="l">提示:</div>
      <div class="r">为了确保邮件投递准确无误，请申请人准确、详细填写收件人信息、联系电话及收件地址，如因信息填写有误造成长时间无法收到邮件的，请打咨询电话：022-24013672</div>
    </div>
    <div class="item" v-show="beforeAddrVisiable">
      <div class="title">常用地址</div>
      <select v-model="dataForm.orderId" @change="changeBefore">
        <option :value="0">请选择</option>
        <option v-for="(item,index) in postAddress" :key="index" :value="item.orderId">{{item.postAddress}}</option>
      </select>
    </div>
    <div class="item">
    ...
    <div class="item" v-show="cityShow">
    ...
    <div class="item">
      ...
      <input type="text" placeholder="输入街道/路、门牌号码" v-model="dataForm.street">
    </div>
    <!--<div class="item">-->
...
<script>
  export default {
    ...
    methods: {
      ...
      getPostAddress() {
        this.$http({
          url: this.$http.adornUrl('/mobile/post/beforePostAddress'),
          method: 'get',
          params: this.$http.adornParams({
          ...
          if (data && data.code === 0) {
            this.postAddress = []
            data.data.forEach((item) => {
              this.postAddress.push({
                postAddress: item.street,
                orderId:item.orderId
              })
              this.beforeAddrVisiable = true
            })
          }
        })
      },
      ...
      changeBefore() {
        ...
        if(this.dataForm.orderId==0)
        {
          ...
          this.dataForm.postProvinceId=''
          this.dataForm.postCityId=''
          this.dataForm.postCountyId=''
        }
        // this.dataForm.street = this.dataForm.addr
        //根据id去查询对应的order信息，把他们填到对应的表单中
        this.$http({
          url: this.$http.adornUrl('/mobile/post/infoForAddr'),
          method: 'get',
          params: this.$http.adornParams({
          ...
          if (data && data.code === 0) {
            ...
            this.dataForm.postProvinceId=data.data.postProvinceId
            this.dataForm.postCityId=data.data.postCityId
            this.dataForm.postCountyId=data.data.postCountyId
            if (data.data.postProvinceName != '新疆维吾尔自治区' && data.data.postProvinceName != '西藏自治区') {
              this.dataForm.postRiskId = this.insuredList[0].id
              this.postPrice = this.insuredList[0].price // 邮寄价格
              this.rateFree = this.insuredList[0].insuredRate // 保险费率价格
            } else {
            ...
            // console.log(this.dataForm.postCityId+"....."+this.dataForm.postCountyId)
          }
        })
      },
      jump4() {
        ...
        if (this.dataForm.postProvinceId == '' || this.dataForm.postCityId == '' || this.dataForm.postCountyId == '' ||
          this.dataForm.postProvinceId == null || this.dataForm.postCityId == null || this.dataForm.postCountyId ==
          null) {
          this.common.myAlert('请输入收件地址', this)
          return
        }
        if (this.dataForm.street == '' || this.dataForm.street == null) {
          this.common.myAlert('请输入街道/路、门牌号码', this)
          return
        } else {
